...
namespace Script.Physics
{
    public class FormulaLibrary
    {
        ...
        public static Vector3 CalculateGravityPull(AstralBodyHandler body, AstralBodyHandler otherBody, Vector3 position, float timeStep = 0)
        {
            ...
            if (otherBody == body) return Vector3.zero;
            
            float distanceFromOther = Vector3.Distance(position, otherBody.transform.position + timeStep * otherBody.Velocity);

           // Debug.Log("[AstralBody] Distance between " + body + " and " + otherBody + " : " + distanceFromOther);
           ...
           // Debug.Log("[AstralBody] Gravitationnal Pull between" + body + " and " + otherBody + " : " + pull +": G: " + G) ;
        ...
        public static double DetermineOrbitingRadius(AstralBodyHandler bodyHandler, float velocity)
        {
            double df = UniverseManager.Instance.PhysicsProperties.DistanceFactor;
        
            velocity *= Mathf.Deg2Rad; //to radian
            velocity *= 10;
        
            var radius = (G * bodyHandler.Mass) * bodyHandler.InfluenceStrength  * bodyHandler.InfluenceStrength * UniverseManager.Instance.PhysicsProperties.DirectGravityPullMultiplier / (velocity * velocity);
            // Debug.Log("radius : " + radius);
            return radius;
        }

        public static float DetermineOrbitingVelocity(AstralBodyHandler bodyHandler, double radius)
        {
            //  r = G * bodyHandler.Mass / (velocity * velocity)*df;
            //v^2 *  = G * bodyHandler.Mass / df * r
            // v = sqrt(G * bodyHandler.Mass / df * r)
            ...
            double innerTerm = (G * bodyHandler.Mass) * bodyHandler.InfluenceStrength  * bodyHandler.InfluenceStrength * UniverseManager.Instance.PhysicsProperties.DirectGravityPullMultiplier /radius;
            float velocity = Mathf.Sqrt((float)innerTerm);
            //TODO : works with bodyHandler.InfluenceStrength == 1000 ...get messy if not ... fix
            velocity /= 10;
